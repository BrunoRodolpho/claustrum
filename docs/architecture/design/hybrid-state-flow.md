...
> **Migrated from ibatexas** (originally `docs/architecture/design/hybrid-state-flow.md` in `BrunoRodolpho/ibatexas`). The reference implementation lived inside the ibatexas monorepo at `packages/llm-provider/`; that package was deleted in the claustrum cutover. The pattern documented here generalises beyond the original ibatexas commerce domain — it describes how a `@claustrum/core`-based adopter can layer a deterministic XState-driven business machine behind a probabilistic LLM-driven natural-language surface, while preserving the kernel-mediated mutation contract (`adjudicate()`).
>
> Concrete file paths referenced below (e.g. `packages/llm-provider/src/...`) describe the **historical ibatexas prototype** of this pattern. They are retained for archaeological context; the canonical interfaces in claustrum are `PromptComposer`, `PlannerPort`, and `ToolRegistry` from `@claustrum/core`.

A conversational adopter built on `@claustrum/core` can implement this pattern as a **10-layer deterministic pipeline** built on XState v5, with verified execution, adaptive runtime, and observability. This document describes why the architecture exists, how each layer behaves, and where the reference implementation lived in ibatexas pre-cutover.
